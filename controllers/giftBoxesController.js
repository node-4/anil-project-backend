...
const createGiftBox = async (req, res) => {
    try {
        ...
        const allProductsExist = await Product.find({ _id: { $in: products }, });
        if (allProductsExist.length !== products.length) {
            return createResponse(res, 400, "Invalid product ID(s)");
        }
        const giftBox = new GiftBox({ name, description, image: { url: req.file.location, key: req.file.key }, price, products, });
        await giftBox.save();
        console.log(giftBox);
        return createResponse(res, 201, "Gift box created successfully", giftBox);
    } catch (error) {
        console.error(error);
...
const getGiftBoxes = async (req, res) => {
    try {
        ...
        return createResponse(res, 200, "Gift boxes retrieved successfully", giftBoxes);
    } catch (error) {
        console.error(error);
        return createResponse(res, 500, "Server error");
    }
...
const getGiftBoxById = async (req, res) => {
    try {
        ...
        const giftBox = await GiftBox.findById(id).populate("products");
        if (!giftBox) {
            return createResponse(res, 404, "Gift box not found");
        }
        return createResponse(res, 200, "Gift box retrieved successfully", giftBox
        );
    } catch (error) {
        console.error(error);
        return createResponse(res, 500, "Server error");
...
const updateGiftBox = async (req, res) => {
    try {
        ...
        let giftBox = await GiftBox.findByIdAndUpdate(id, req.body, { new: true, });
        if (!giftBox) {
            return createResponse(res, 404, "Gift box not found");
        }
        return createResponse(res, 200, "Gift box updated successfully", giftBox
        );
    } catch (error) {
        console.error(error);
        return createResponse(res, 500, "Server error");
